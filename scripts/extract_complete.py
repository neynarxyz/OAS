...
for category in sorted(schema_categories.keys()):
    new_spec += f"    # {category.capitalize()} schemas\n"
    new_spec += f"    $ref: \"./components/schemas/{category}.yaml\"\n"
...
with open("src/v2/spec.yaml.new", 'w') as f:
    f.write(new_spec)

print("Created new spec file at src/v2/spec.yaml.new")
